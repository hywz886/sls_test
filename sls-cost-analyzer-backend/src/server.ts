...
import { analyzeWithLLM, saveAnalysisReport, loadAnalysisReport, type DailyAnalysisReport } from './aiAnalyzer';
...
const processDailyData = async (
  ...
  const dayAvgResult = await getAverageCostByWorkdayType(historicalData, date, 1, isCurrentWorkday);
  ...
  let dayComparison = null;
  let weekComparison = null;
  let monthComparison = null;

  if (dayAvgResult) {
    dayComparison = calculateComparison(totalCost, dayAvgResult.avgTotalCost);
  }
  ...
  const logstoreSummariesWithComparison = Array.from(logstoreMap.values()).map((item: LogstoreData) => {
    ...
    if (item.totalCost >= 10) {
      ...
      // 添加 Logstore 级别的日对比 (基于前 1 个同类型日期的平均)
      if (dayAvgResult?.avgLogstoreCosts?.has(key)) {
        const dayAvgCost = dayAvgResult.avgLogstoreCosts.get(key)!;
        logstoreData.dayComparison = calculateComparison(item.totalCost, dayAvgCost);
      } else {
        logstoreData.dayComparison = null;
      }
  ...
  // 🤖 调用大模型进行日志数据分析
  try {
    console.log(`🤖 开始 AI 日志分析:${date}`);

    const analysisReport = await analyzeWithLLM(
      date,
      totalCost,
      dayComparison,
      weekComparison,
      monthComparison,
      logstoreSummariesWithComparison
    );

    // 保存分析报告
    await saveAnalysisReport(analysisReport, targetDir);

    console.log(`✅ AI 分析完成:${date}`);
  } catch (error) {
    console.error(`❌ AI 分析失败:${date}`, error);
    // 不阻塞主流程,继续返回
  }
...
app.get('/api/query', async (req, res) => {
  try {
    ...
    if (dailyCosts.length > 0) {
      ...
      // 精确匹配 sls-cost-YYYY-MM-DD.json 文件，排除 -analysis.json
      const latestFile = files.find(f => f === `sls-cost-${latestDate}.json`);
      if (latestFile) {
    ...
    if (dailyCosts.length === 1 && dailyCosts[0].date) {
      ...
      if (currentData) {
        ...
        console.log(`🔍 计算对比数据:${singleDate}, isWorkday=${isCurrentWorkday}, historicalData.size=${historicalData.size}`);

        // 获取前 1 个同类型日期的平均 (日对比)
        const dayAvgResult = await getAverageCostByWorkdayType(historicalData, singleDate, 1, isCurrentWorkday);
        console.log(`📊 dayAvgResult:`, dayAvgResult ? `avgTotalCost=${dayAvgResult.avgTotalCost.toFixed(2)}, matchedDates=${dayAvgResult.matchedDates.length}` : 'null');
        if (dayAvgResult) {
          dayComparison = calculateComparison(totalCost, dayAvgResult.avgTotalCost);
          console.log(`✅ dayComparison: ${dayComparison.changePercent.toFixed(2)}%`);
        }

        // 获取前 7 个同类型日期的平均
        const weekAvgResult = await getAverageCostByWorkdayType(historicalData, singleDate, 7, isCurrentWorkday);
        console.log(`📊 weekAvgResult:`, weekAvgResult ? `avgTotalCost=${weekAvgResult.avgTotalCost.toFixed(2)}, matchedDates=${weekAvgResult.matchedDates.length}` : 'null');
        if (weekAvgResult) {
          weekComparison = calculateComparison(totalCost, weekAvgResult.avgTotalCost);
          console.log(`✅ weekComparison: ${weekComparison.changePercent.toFixed(2)}%`);
        }

        // 获取前 30 个同类型日期的平均
        const monthAvgResult = await getAverageCostByWorkdayType(historicalData, singleDate, 30, isCurrentWorkday);
        console.log(`📊 monthAvgResult:`, monthAvgResult ? `avgTotalCost=${monthAvgResult.avgTotalCost.toFixed(2)}, matchedDates=${monthAvgResult.matchedDates.length}` : 'null');
        if (monthAvgResult) {
          monthComparison = calculateComparison(totalCost, monthAvgResult.avgTotalCost);
          console.log(`✅ monthComparison: ${monthComparison.changePercent.toFixed(2)}%`);
        }
      } else {
        console.log(`❌ currentData not found for ${singleDate}`);
      }
    }
  ...
  try {
    ...
    if (!date) {
      return res.status(400).json({
        success: false,
        error: '缺少必需参数:date',
        hint: '请传入日期,格式:YYYY-MM-DD,例如:2026-04-21',
      });
    }

    // 尝试从文件加载分析报告
    const report = loadAnalysisReport(date, DATA_DIR);

    if (!report) {
      return res.status(404).json({
        success: false,
        error: '未找到分析报告',
        hint: '该日期的数据可能尚未进行 AI 分析',
      });
    }
  ...
  watch(SOURCE_DIR, (eventType, filename) => {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(`   - 🤖 AI 分析报告：sls-cost-${date}-analysis.json`);
...
app.listen(PORT, () => {
  ...
  console.log(`  - GET  /api/analysis         - 🤖 获取 AI 分析报告`);
